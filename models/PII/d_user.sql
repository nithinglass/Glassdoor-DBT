with
    users as (select * from {{ ref("gd_user") }}),
    pseudonymized as (
        select
            user_id,
            {{ dbt_utils.surrogate_key([user_id]) }} as user_key, 
            {{ dbt_privacy.hash("name", salt_expr="user_id") }} as name_hash,
            {{ dbt_privacy.hash("email", salt_expr="user_id") }} as email_hash,
            {{ dbt_privacy.hash("email", salt_expr="user_id", digest_size=512) }}
            as email_hash_512,
            {{ dbt_privacy.hash_unique("email", salt_expr="user_id") }} as unique_email_hash,
